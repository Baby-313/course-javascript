...
export default {
  ...
  handleEvents() {
    ...
    document 
      ...
      .addEventListener('click', async()=>{
        const{liked,likes}=await model.like(this.photoId);
        this.setLikes(liked,likes);
      });

    document
      .querySelector('.component-footer-container-social-comments')
      .addEventListener('click', async ()=>{
        document.querySelector('.component-comment').classList.remove('hidden');
        await this.loadComments(this.photoId)
      });
    
    const input=document.querySelector('.component-comments-container-form-input');

    document.querySelector('.component-comment').addEventListener('click',(e)=>{
      if (e.target === e.currentTarget) {
        document.querySelector('.component-comment').classList.add('hidden');
      }
    });
